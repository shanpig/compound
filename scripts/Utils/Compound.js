...
async function makeComptroller(opts = {}) {
  const { root = process.env.ACCOUNT_ADDRESS, kind = 'unitroller' } =
    opts || {};
...
async function makeCToken(opts = {}) {
  const { root = process.env.ACCOUNT_ADDRESS, kind = 'cerc20' } = opts || {};
...
async function makeInterestRateModel(opts = {}) {
  const { root = process.env.ACCOUNT_ADDRESS, kind = 'harnessed' } = opts || {};
...
async function makePriceOracle(opts = {}) {
  const { root = process.env.ACCOUNT_ADDRESS, kind = 'simple' } = opts || {};
...
async function makeToken(opts = {}) {
  const { root = process.env.ACCOUNT_ADDRESS, kind = 'erc20' } = opts || {};
...
async function setEtherBalance(cEther, balance) {
  ...
  const root = process.env.ACCOUNT_ADDRESS;
  expect(
