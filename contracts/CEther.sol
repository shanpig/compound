...
import "./CToken.sol";
import "./NFT_contracts/CERC721.sol";
...
contract CEther is CToken {
    ...
    function ERC721_LiquidateBorrow(
        address borrower,
        uint8 tokenId,
        CERC721Interface cERC721Collateral
    ) external payable {
        (uint256 err, ) = ERC721_LiquidateBorrowInternal(
            borrower,
            tokenId,
            cERC721Collateral
        );
        requireNoError(err, "liquidateBorrow with nft failed");
    }

    function ERC721_LiquidateBorrowInternal(
        address borrower,
        uint8 tokenId,
        CERC721Interface cERC721Collateral
    ) internal nonReentrant returns (uint256, uint256) {
        return
            ERC721_LiquidateBorrowFresh(
                msg.sender,
                borrower,
                tokenId,
                cERC721Collateral
    ...
    function ERC721_LiquidateBorrowFresh(
        address liquidator,
        address borrower,
        uint8 tokenId,
        CERC721Interface cERC721Collateral
    ) internal returns (uint256, uint256) {
        cERC721Collateral.seize(liquidator, borrower, tokenId);

        return (uint256(Error.NO_ERROR), uint256(1));
    }
